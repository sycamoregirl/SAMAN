const Nutricion = (() => {
    ...
    function clasificarDesnutricion(peso, talla, perimetroBrazo, zScore) {
        if (zScore !== null && zScore !== undefined && isFinite(zScore)) {
            if (zScore >= 0) return 'Normal';
            if (zScore < -3) return 'Severa';
            if (zScore < -2) return 'Moderada';
            return 'Leve';
        }
